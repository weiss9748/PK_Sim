...
eps=1E-10; %epsilon, predefined error control criterion
C0=((B./(A.*La)).*n0).'; %initial delayed neutron precursor densities for all the groups
y0=[n0;C0]; %initial y column vector
yall=zeros(NGr+3,length(ti));
yall(:,1)=[0;h(1);y0];
y=ones(NGr+1,1);
g1=zeros(NGr+1,length(ti));
g2=zeros(NGr+1,length(ti));
g3=zeros(NGr+1,length(ti));
g4=zeros(NGr+1,length(ti));
...
FIg1(:,i)=figure('Name','Neutron Density','NumberTitle','off');
...
FIg2(:,i)=figure('Name','Delayed Neutron Precursor Density','NumberTitle','off');
%uncomment line directly above to enable seperate plots
%subplot(1,2,2)
...
while max(ti)<max(IPts)
    ...
    dfdy=[((double(subs(rho,ti(i)))-Beta)/A) La; (B(1)/A) -La;
        ...
        (B(5)/A) -La;
        (B(6)/A) -La];
    dfdt=[((double(subs(drho,ti(i)))*(yall(3,i-1)))/A);0;0;0;0;0;0]; %df/dt
    [L,U]=lu(((1./(gam*h(i-1))))-dfdy); %LU decompisition of LHS of eqns 3 in the paper, based on h=0.01
    g1(:,i)=((([((double(subs(rho,ti(i-1)))-Beta)/A) La; (B(1)/A) -La;
        (B(2)/A) -La;
        (B(3)/A) -La;
        (B(4)/A) -La;
        (B(5)/A) -La;
        (B(6)/A) -La])*fpre)+(h(i-1).*c1.*dfdt))/norm(L*U); %g1(:,i) initial calculation
    g2(:,i)=((([((double(subs(rho,(ti(i-1)+a2*h(i-1))))-Beta)/A) La; (B(1)/A) -La;
        (B(2)/A) -La;
        (B(3)/A) -La;
        (B(4)/A) -La;
        (B(5)/A) -La;
        (B(6)/A) -La])*(fpre+(a21*g1(:,i))))+(h(i-1).*c2.*dfdt)+((c21*g1(:,i))/h(i-1)))/norm(L*U); %g2(:,i) initial calculation
    g3(:,i)=((([((double(subs(rho,(ti(i-1)+a3*h(i-1))))-Beta)/A) La; (B(1)/A) -La;
        (B(2)/A) -La;
        (B(3)/A) -La;
        (B(4)/A) -La;
        (B(5)/A) -La;
        (B(6)/A) -La])*(fpre+(a31*g1(:,i))+(a32*g2(:,i))))+(h(i-1).*c3.*dfdt)+(((c31*g1(:,i))+(c32*g2(:,i)))/h(i-1)))/norm(L*U);%g3(:,i) initial calculation
    g4(:,i)=((([((double(subs(rho,(ti(i-1)+a3*h(i-1))))-Beta)/A) La; (B(1)/A) -La;
        (B(2)/A) -La;
        (B(3)/A) -La;
        (B(4)/A) -La;
        (B(5)/A) -La;
        (B(6)/A) -La])*(fpre+(a31*g1(:,i))+(a32*g2(:,i))))+(h(i-1).*c4.*dfdt)+(((c41*g1(:,i))+(c42*g2(:,i))+(c43*g3(:,i))))/h(i-1))/norm(L*U); %g4(:,i) initial calculation
    err=(e1*g1(:,i))+(e2*g2(:,i))+(e3*g3(:,i))+(e4*g4(:,i)); %initial truncation error calculations
    ...
    if h(i)>ti(i-1)
        ix = find(IPts>ti(i-1),1);
        h(i)=abs(IPts(ix)-ti(i-1));
        ti(i)=ti(i-1)+h(i);
        dfdy=[((double(subs(rho,ti(i)))-Beta)/A) La; (B(1)/A) -La;
            (B(2)/A) -La;
            (B(3)/A) -La;
    ...
    y=y0+((b1*g1(:,i))+(b2*g2(:,i))+(b3*g3(:,i))+(b4*g4(:,i)));
    yall(:,i)=[ti(i);h(i);y];
    n=y(1);
    C1=y(2);
    C2=y(3);
