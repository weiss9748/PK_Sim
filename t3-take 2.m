...
while max(ti)<max(IPts)
    ...
    g1(:,i)=(f0+(h(i-1).*c1.*dfdt))/norm(L*U); %g1(:,i) initial calculation
    g2(:,i)=((([((double(subs(rho,(a2*h(i-1))))-Beta)/A) La; (B(1)/A) -La;
        (B(2)/A) -La;
        (B(3)/A) -La;
        (B(4)/A) -La;
        (B(5)/A) -La;
        (B(6)/A) -La])*(y0+(a21*g1(:,i))))+(h(i-1).*c2.*dfdt)+((c21*g1(:,i))/h(i-1)))/norm(L*U); %g2(:,i) initial calculation
    g3(:,i)=((([((double(subs(rho,(a3*h(i-1))))-Beta)/A) La; (B(1)/A) -La;
        (B(2)/A) -La;
        (B(3)/A) -La;
        (B(4)/A) -La;
        (B(5)/A) -La;
        (B(6)/A) -La])*(y0+(a31*g1(:,i))+(a32*g2(:,i))))+(h(i-1).*c3.*dfdt)+(((c31*g1(:,i))+(c32*g2(:,i)))/h(i-1)))/norm(L*U);%g3(:,i) initial calculation
    g4(:,i)=((([((double(subs(rho,(a3*h(i-1))))-Beta)/A) La; (B(1)/A) -La;
        (B(2)/A) -La;
        (B(3)/A) -La;
        (B(4)/A) -La;
        (B(5)/A) -La;
        (B(6)/A) -La])*(y0+(a31*g1(:,i))+(a32*g2(:,i))))+(h(i-1).*c4.*dfdt)+(((c41*g1(:,i))+(c42*g2(:,i))+(c43*g3(:,i))))/h(i-1))/norm(L*U); %g4(:,i) initial calculation
